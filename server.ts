...
interface ReservationPerDayRow {
  perday_id: number;
  res_id: number; // common link field
  detail_id: number;
  rate_date: string;
...
const initialReservationPerDay: ReservationPerDayRow[] = [
  { perday_id: 801, res_id: 9001, detail_id: 501, rate_date: '2026-09-20 00:00:00', roomtypeid: 101, room_rate: 4500.00, tax_amount: 1000.00, total_amount: 5500.00 },
  { perday_id: 802, res_id: 9002, detail_id: 502, rate_date: '2026-09-21 00:00:00', roomtypeid: 102, room_rate: 7500.00, tax_amount: 2000.00, total_amount: 9500.00 },
  { perday_id: 803, res_id: 9003, detail_id: 503, rate_date: '2026-09-22 00:00:00', roomtypeid: 201, room_rate: 11000.00, tax_amount: 4000.00, total_amount: 15000.00 },
  { perday_id: 804, res_id: 9004, detail_id: 504, rate_date: '2026-09-25 00:00:00', roomtypeid: 301, room_rate: 14000.00, tax_amount: 5000.00, total_amount: 19000.00 },
];
...
const state = {
  ...
  localSqlServer: {
    ...
    Reservation_PerDay_details_Booklogic: [] as ReservationPerDayRow[],
  ...
  virtualServerPostgres: {
    ...
    reservation_perday_details: [...initialReservationPerDay],
...
async function startServer() {
  ...
  app.get('/api/data/compare', (req, res) => {
    res.json({
      ...
      Reservation_PerDay_details_Booklogic: {
        tableName: 'Reservation_PerDay_details_Booklogic',
        sourceCount: state.localSqlServer.Reservation_PerDay_details_Booklogic.length,
        targetCount: state.virtualServerPostgres.reservation_perday_details.length,
        inSync: state.localSqlServer.Reservation_PerDay_details_Booklogic.length === state.virtualServerPostgres.reservation_perday_details.length,
        sourceRows: state.localSqlServer.Reservation_PerDay_details_Booklogic,
        targetRows: state.virtualServerPostgres.reservation_perday_details,
      },
...
`;
  ...
  app.post('/api/sync-unit/inbound-sync-reservations', async (req, res) => {
    ...
    if (password || host === '72.61.240.34') {
      try {
        ...
        // Ensure 4 source tables exist on VPS PostgreSQL
        await client.query(`
          ...
          CREATE TABLE IF NOT EXISTS public.reservation_perday_details (
            perday_id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
            res_id BIGINT REFERENCES public.reservations(res_id) ON DELETE CASCADE,
            detail_id BIGINT,
            rate_date TIMESTAMP WITHOUT TIME ZONE,
            roomtypeid BIGINT,
            room_rate NUMERIC(12, 2) DEFAULT 0.00,
            tax_amount NUMERIC(12, 2) DEFAULT 0.00,
            total_amount NUMERIC(12, 2) DEFAULT 0.00
          );
        ...
        if (pgRes.rows && pgRes.rows.length > 0) {
          ...
          // Fetch matching records from reservation_perday_details
          let perdayRes = { rows: [] as any[] };
          try {
            perdayRes = await client.query(`
              SELECT * FROM public.reservation_perday_details WHERE res_id = ANY($1::bigint[]);
            `, [resIds]);
          } catch (e) {}
          ...
          if (perdayRes.rows && perdayRes.rows.length > 0) {
            for (const p of perdayRes.rows) {
              const perdayRow: ReservationPerDayRow = {
                perday_id: Number(p.perday_id || p.id),
                res_id: Number(p.res_id),
                detail_id: Number(p.detail_id || p.res_id * 1000 + 1),
                rate_date: p.rate_date ? new Date(p.rate_date).toISOString().replace('T', ' ').substring(0, 19) : '',
                roomtypeid: Number(p.roomtypeid || 101),
                room_rate: Number(p.room_rate || 0),
                tax_amount: Number(p.tax_amount || 0),
                total_amount: Number(p.total_amount || 0),
                _synced_at: new Date().toISOString(),
              };

              const existingIdx = state.localSqlServer.Reservation_PerDay_details_Booklogic.findIndex(x => x.perday_id === perdayRow.perday_id);
              if (existingIdx >= 0) {
                state.localSqlServer.Reservation_PerDay_details_Booklogic[existingIdx] = perdayRow;
              } else {
                state.localSqlServer.Reservation_PerDay_details_Booklogic.unshift(perdayRow);
              }
            }
          }
    ...
    if (syncedReservationsCount === 0 && pendingVirtualReservations.length > 0) {
      ...
      for (const r of pendingVirtualReservations) {
        ...
        // Upsert matching perday into Reservation_PerDay_details_Booklogic
        const matchingPerDay = state.virtualServerPostgres.reservation_perday_details.filter(p => p.res_id === r.res_id);
        for (const p of matchingPerDay) {
          const perdayRow: ReservationPerDayRow = { ...p, _synced_at: new Date().toISOString() };
          const pIdx = state.localSqlServer.Reservation_PerDay_details_Booklogic.findIndex(x => x.perday_id === p.perday_id);
          if (pIdx >= 0) {
            state.localSqlServer.Reservation_PerDay_details_Booklogic[pIdx] = perdayRow;
          } else {
            state.localSqlServer.Reservation_PerDay_details_Booklogic.unshift(perdayRow);
          }
        }
      ...
      for (const p of state.virtualServerPostgres.reservation_perday_details) {
        state.localSqlServer.Reservation_PerDay_details_Booklogic.push({ ...p, _synced_at: new Date().toISOString() });
      }
    ...
    res.json({
      ...
      perday: state.localSqlServer.Reservation_PerDay_details_Booklogic,
      customers: state.localSqlServer.reservation_Customer_booklogic,
      message: `Successfully synchronized ${syncedReservationsCount} Reservations, ${syncedDetailsCount} details, per-day details, and ${syncedCustomersCount} customers (link: res_id) where updateflag = 0 from PostgreSQL to local SQL Server (Reservations_booklogic, reservations_details_booklogic, Reservation_PerDay_details_Booklogic, reservation_Customer_booklogic)!`,
    });
  });
  ...
  app.post('/api/sync-unit/create-reservation-pg', async (req, res) => {
    ...
    const newPerDay: ReservationPerDayRow = {
      perday_id: 800 + state.virtualServerPostgres.reservation_perday_details.length + 1,
      res_id: nextResId,
      detail_id: nextDetailId,
      rate_date: check_in,
      roomtypeid: Number(roomtypeid),
      room_rate: Number(price_per_night),
      tax_amount: Number(total_amount) * 0.12,
      total_amount: Number(total_amount),
    };
    ...
    state.virtualServerPostgres.reservation_perday_details.unshift(newPerDay);
    ...
    if (password) {
      try {
        ...
        await client.query(`
          INSERT INTO public.reservation_perday_details (
            perday_id, res_id, detail_id, rate_date, roomtypeid, room_rate, tax_amount, total_amount
          ) OVERRIDING SYSTEM VALUE VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
          ON CONFLICT (perday_id) DO NOTHING;
        `, [
          newPerDay.perday_id,
          nextResId,
          nextDetailId,
    ...
    res.json({
      ...
      directRemoteSuccess,
      directRemoteError,
      message: `Created Relational Reservation #${nextResId} (${first_name} ${last_name}) in PostgreSQL [Reservations, reservations_details, reservation_perday_details, reservation_Customer] with updateflag = 0. Ready for inbound sync into local SQL Server!`,
    });
  });
  ...
  app.get('/api/sync-agent-code', (req, res) => {
    try {
      ...
      if (fs.existsSync(filePath)) {
        const content = fs.readFileSync(filePath, 'utf8');
        res.json({ success: true, code: content });
      } else {
        res.status(404).json({ success: false, message: 'sync_agent.js not found' });
      }
    } catch (err: unknown) {
